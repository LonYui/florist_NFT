<template>
  <div id="vueRoot">
    <form ref="form" method="post" v-bind:action="action_url" >
...
export default {
  ...
  methods : {
    ...
    fetch_create_todopayment(member_id,amount){
      var formdata = new FormData();
      formdata.append("amount", amount);
      formdata.append("to", member_id);
  ...
  computed:{
    verify(){
      let str_to_md5 = `${process.env.VUE_APP_open_pay_apikey1}|${this.mid}|${this.txid}|${this.amount}|${process.env.VUE_APP_open_pay_apikey2}`
      return md5(str_to_md5)
      //ref stackover flow js md5
    ...
    return_url(){
      return `${window.location.origin}/tabs/tab3?member_id=${this.member_id}&txn_id=${this.txid}`
    }
  },
  async mounted(){
    const _this = this
