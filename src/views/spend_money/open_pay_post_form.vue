...
export default {
  ...
  data : {
    ...
    amount :'1688',
    txid :'',
    return_url :`${window.location.origin}/tabs/tab3?member_id=${member_id}`,
    language :'tchinese',
  },
  methods : {
    submit : function(){
    ...
    fetch_create_todopayment(txid,member_id,amount){

    }
  },
  computed:{
    verify(){
      let str_to_md5 = `11111222223333344444555556666677|${mid}|${txid}|${amount}|11111222223333344444555556666688`
  ...
  mounted(){
    const _this = this
    fetch_create_todopayment(this.member_id,this.amount).then(response => {
      response.json().then(json => {
        _this.txid = json['payment_id']
        _this.submit()
      })
    })
  },
